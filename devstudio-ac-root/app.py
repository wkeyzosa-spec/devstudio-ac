...
def api_ban():
    ...
    if data.get('unban'):
        ban_obj = Ban.query.filter_by(ban_id=data.get('ban_id')).first()
...
def api_server_config(server_id):
    if request.method == 'POST':
        ...
        return jsonify({'success': True})
    cfg = ServerConfig.query.filter_by(server_id=server_id).first()
    return jsonify(json.loads(cfg.config_data) if cfg else {})
...
def api_sync_admins(server_id):
    ...
    for a in data:
        sa = ServerAdmin(server_id=server_id, player_id=a.get('id', 0),
